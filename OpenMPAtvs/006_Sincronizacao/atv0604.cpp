...
int main() {
    ...
    #pragma omp parallel 
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            #pragma omp for ordered
            /*
            OMP Ordered organiza a execução do bloco de codigo por ordem de seu indice. Assim as threads executam o codigo
            na ordem do indice do for no codigo abaixo 
            */

            for (int i = 0; i < 16; ++i) { //Reduzi de 1000 para 16 para facilitar a visualização
            printf("Indice %i verificando raizes salvas: {%.2f, %.2f}.\n", i, raizesTotais[i].first, raizesTotais[i].second );
            }
